...
<select name="movie_genre" onchange="location = this.value;"
    class="block w-full mb-4 border-gray-300 rounded-md shadow-sm dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600"
    required>
    <option>{{ isset($currentGenre) ? ucwords($currentGenre->name) : 'Genres' }}</option>
    <hr />
    <option value="/archive/movies/" :active="request() - > routeIs('/')">All</option>
    @foreach (\App\Models\Genre::all()->sortBy('name') as $genre)
        <option value="/archive/movies/?genre={{ $genre->name }}"
...
{{-- <x-dropdown>
    ...
    <x-dropdown.link href="/archive/movies/?{{ http_build_query(request()->except('genre', 'page')) }}" :active="request()->routeIs('/archive/movies')">
        All
    </x-dropdown.link>

    @foreach ($genres->sortBy('name') as $genre)
        <x-dropdown.link href="/archive/movies/?genre={{ $genre->name }}&{{ http_build_query(request()->except('genre', 'page')) }}"
            :active='request()->is("/archive/movies/?genre={$genre->name}")'>
            {{ ucwords($genre->name) }}
            </x-dropdown.link>
    @endforeach
